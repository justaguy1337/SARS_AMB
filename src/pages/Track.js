...
const Track = () => {
  ...
  const calculateRoutes = () => {
    ...
    const mockAmbulances = [
      { id: 'AMB-001', lat: 28.6350, lng: 77.2250, crew: 4, status: 'available' },
      { id: 'AMB-002', lat: 28.6100, lng: 77.1950, crew: 3, status: 'available' },
      { id: 'AMB-003', lat: 28.6400, lng: 77.2300, crew: 4, status: 'available' },
      { id: 'AMB-004', lat: 28.6050, lng: 77.2150, crew: 2, status: 'available' },
      { id: 'AMB-005', lat: 28.6280, lng: 77.2180, crew: 3, status: 'available' },
    ];
  ...
  const initializeMap = () => {
    ...
    map.on('load', () => {
      ...
      ambulanceRoutes.forEach((route) => {
        ...
        }, `route-${route.ambulanceId}`); // Place outline behind the main route
  ...
  const handleAutoDispatch = () => {
    ...
    setTimeout(() => {
      ...
      const newEmergency = {
        ...
        timeEstimatedArrival: selectedAmbulance.eta,
        status: 'dispatched',
    ...
    <Box sx={{ display: 'flex', minHeight: '100vh', bgcolor: '#f3f4f6' }}>
      ...
      >
        ...
        {isAnalyzing ? (
          ...
          >
            <Typography variant="body2" fontWeight={600}>
              AI Recommendation: {selectedAmbulance.ambulanceId} is the optimal choice with {selectedAmbulance.eta} min ETA
            </Typography>
          </Alert>
        )}
        ...
        <Box sx={{ mb: 3 }}>
          ...
          <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: '1fr 1fr', lg: '1fr 1fr 1fr 1fr 1fr' }, gap: 2 }}>
            {ambulanceRoutes.map((route, index) => (
              <Card
                key={route.ambulanceId}
                sx={{
                  cursor: 'pointer',
                  border: selectedAmbulance?.ambulanceId === route.ambulanceId 
                    ? `3px solid ${route.color}` 
                    : '1px solid #e5e7eb',
                  bgcolor: selectedAmbulance?.ambulanceId === route.ambulanceId 
                    ? '#f0fdf4' 
                    : 'white',
                  transition: 'all 0.3s ease',
                  transform: selectedAmbulance?.ambulanceId === route.ambulanceId 
                    ? 'scale(1.05)' 
                    : 'scale(1)',
                  boxShadow: selectedAmbulance?.ambulanceId === route.ambulanceId 
