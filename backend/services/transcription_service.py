...
class TranscriptionService:
    ...
    async def transcribe_audio(self, audio_file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            transcribed_text = transcript_response.text if hasattr(transcript_response, 'text') else ""
...
4. emergency_type: SPECIFIC type (e.g., "Heart Attack", "Severe Car Accident", "Respiratory Failure", "Stroke")
5. symptoms: Comprehensive list of all symptoms mentioned or implied (array of strings)
6. location: Full address if mentioned, otherwise describe location from context
7. severity: Based on symptoms - "critical" (life-threatening), "high" (serious), "medium" (urgent), "low" (non-urgent)
8. caller_name: Caller's name if mentioned, otherwise use relationship (e.g., "Spouse", "Neighbor", "Witness")
9. caller_phone: Phone number if mentioned, otherwise empty string
10. special_requirements: Medical equipment/support needed based on emergency type (array, e.g., ["Cardiac Care", "Oxygen Support"])
11. consciousness: Patient conscious state (yes/no/unknown) - infer from description if not explicit
12. breathing: Breathing status (yes/no/difficulty/unknown) - infer from description if not explicit
...
"""
            ...
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are an expert medical AI for emergency services. Extract information from calls and intelligently fill missing details using medical knowledge. Always provide specific, actionable information. Return ONLY valid JSON."
                    },
                    {
                        "role": "user",
    ...
    def _ensure_complete_data(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not data.get("special_requirements"):
            ...
            if "heart" in emergency_type or "cardiac" in emergency_type:
                data["special_requirements"] = ["Cardiac Care", "Defibrillator"]
            elif "breath" in emergency_type or "respiratory" in emergency_type:
                data["special_requirements"] = ["Oxygen Support", "Respiratory Care"]
            elif "accident" in emergency_type or "trauma" in emergency_type:
                data["special_requirements"] = ["Trauma Care", "Multiple Units"]
            elif "stroke" in emergency_type:
                data["special_requirements"] = ["Stroke Protocol", "Critical Care"]
            else:
